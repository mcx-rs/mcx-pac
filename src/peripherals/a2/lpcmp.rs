...
pub mod regs {
    ...
    impl core::fmt::Debug for RRCR0 {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("RRCR0")
                ...
                .field("RR_EXTTRG_SEL", &self.RR_EXTTRG_SEL())
    ...
    impl defmt::Format for RRCR0 {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "RRCR0 {{ RR_EN: {=bool:?}, RR_TRG_SEL: {=bool:?}, RR_EXTTRG_SEL: {=u8:?}, RR_NSAM: {=u8:?}, RR_CLK_SEL: {=u8:?}, RR_INITMOD: {=u8:?}, RR_SAMPLE_CNT: {=u8:?}, RR_SAMPLE_THRESHOLD: {=u8:?} }}" , self . RR_EN () , self . RR_TRG_SEL () , self . RR_EXTTRG_SEL () , self . RR_NSAM () , self . RR_CLK_SEL () , self . RR_INITMOD () , self . RR_SAMPLE_CNT () , self . RR_SAMPLE_THRESHOLD ())
        }
    }
