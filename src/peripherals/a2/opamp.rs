...
impl OPAMP {
    ...
    #[inline(always)]
    pub const fn OPAMP_CTRL(self) -> crate::common::Reg<regs::OPAMP_CTRL, crate::common::RW> {
...
pub mod regs {
    ...
    impl Default for OPAMP_CTRL {
        #[inline(always)]
        fn default() -> OPAMP_CTRL {
            OPAMP_CTRL(0)
        }
    }
    ...
    impl defmt::Format for OPAMP_CTRL {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "OPAMP_CTRL {{ OPA_EN: {=bool:?}, OPA_CC_SEL: {=u8:?}, OPA_BC_SEL: {=u8:?} }}",
                self.OPA_EN(),
                self.OPA_CC_SEL(),
                self.OPA_BC_SEL()
            )
