...
pub mod opamp {
    ...
    pub const ADDRESSES: [usize; LEN] = [0x4011_0000usize, 0x4011_3000usize, 0x4011_5000usize];
...
pub mod sinc {
    ...
    pub const unsafe fn instance(n: u8) -> Option<SINC> {
        if n >= LEN as u8 {
            None
        } else {
            Some(SINC::from_ptr(ADDRESSES[n as usize] as _))
        }
    }
    pub type Instance<const N: u8> = crate::Instance<SINC, N>;
    impl<const N: u8> Instance<N> {
        const CHECK: () = assert!((N as usize) < LEN);
        #[inline(always)]
        pub const unsafe fn regs() -> SINC {
            let _ = Self::CHECK;
            SINC::from_ptr(Self::address() as _)
        }
        #[inline(always)]
        pub const fn address() -> usize {
            let _ = Self::CHECK;
...
pub mod bsp32 {
    ...
    pub use _block::*;
    pub const LEN: usize = 1usize;
    pub const ADDRESSES: [usize; LEN] = [0x4003_2000usize];
    pub const unsafe fn instance(n: u8) -> Option<BSP32> {
        if n >= LEN as u8 {
            None
        } else {
            Some(BSP32::from_ptr(ADDRESSES[n as usize] as _))
        }
    }
    pub type Instance<const N: u8> = crate::Instance<BSP32, N>;
    impl<const N: u8> Instance<N> {
        const CHECK: () = assert!((N as usize) < LEN);
        #[inline(always)]
        pub const unsafe fn regs() -> BSP32 {
            let _ = Self::CHECK;
            BSP32::from_ptr(Self::address() as _)
        }
        #[inline(always)]
        pub const fn address() -> usize {
            let _ = Self::CHECK;
...
pub mod lpdac {
    ...
    pub use _block::*;
    pub const LEN: usize = 2usize;
    pub const ADDRESSES: [usize; LEN] = [0x4010_f000usize, 0x4011_2000usize];
...
pub mod lpcmp {
    ...
    pub use _block::*;
    pub const LEN: usize = 3usize;
    pub const ADDRESSES: [usize; LEN] = [0x4005_1000usize, 0x4005_2000usize, 0x4005_3000usize];
...
pub mod hpdac {
    ...
    pub use _block::*;
    pub const LEN: usize = 1usize;
    pub const ADDRESSES: [usize; LEN] = [0x4011_4000usize];
    pub const unsafe fn instance(n: u8) -> Option<HPDAC> {
        if n >= LEN as u8 {
            None
        } else {
            Some(HPDAC::from_ptr(ADDRESSES[n as usize] as _))
        }
    }
    pub type Instance<const N: u8> = crate::Instance<HPDAC, N>;
    impl<const N: u8> Instance<N> {
        const CHECK: () = assert!((N as usize) < LEN);
        #[inline(always)]
        pub const unsafe fn regs() -> HPDAC {
            let _ = Self::CHECK;
            HPDAC::from_ptr(Self::address() as _)
        }
        #[inline(always)]
        pub const fn address() -> usize {
            let _ = Self::CHECK;
